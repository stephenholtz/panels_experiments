% Extracted from John's vel null making... (cleaned up)
...
for ref = 7                         % reference peak brightness
    for index = 1:3                 % number of contrast levels
        ...
        test_base = 2*mean_lum-test;% dimmer layer of test
        ...
        for j = 1:12
            ...
            SPats(:,:,1,j) = [repmat([test*ones(4,6), test_base*ones(4,6)], 1, 8)];%%%x is the test pattern
        end
        ...
        str = [directory_name '\Pattern_nulling_6wide_rotation_' num2str(ref) num2str(ref_base) '_' num2str(test) num2str(test_base)];
        save(str, 'pattern');
