...
% Make a horzontal bar (the regular closed loop stripe fixation one)
%==========================================================================
    count = 999;
    pattern_str = '8px_vert_bar_closed_loop_interspersal_pat';
    ...
    base_vert_pat = [low_gs_value*ones(num_rows,vert_stripe_size), high_gs_value*ones(num_rows,num_cols-vert_stripe_size)];
    ...
    vert_pat = [];
    % Create a vertical bar matrix
    base_vert_pat = [low_gs_value*ones(num_rows,vert_stripe_size), mid_gs_value*ones(num_rows,num_cols-vert_stripe_size)];
    % Vertically center the pattern at 47?
    base_vert_pat = circshift(base_vert_pat,[0 -2-vert_stripe_size/2]);
    for i = 1:num_cols
        vert_pat(:,:,i) = circshift(base_vert_pat,[0 i-1]); %#ok<*SAGROW>
    ...
    % Low contrast dark bar
    Pats(:,:,:,2) = vert_pat;
    
    
    vert_pat = [];
    % Create a vertical bar matrix
    base_vert_pat = [high_gs_value*ones(num_rows,vert_stripe_size), low_gs_value*ones(num_rows,num_cols-vert_stripe_size)];
    % Vertically center the pattern at 47?
    base_vert_pat = circshift(base_vert_pat,[0 -2-vert_stripe_size/2]);
    for i = 1:num_cols
        vert_pat(:,:,i) = circshift(base_vert_pat,[0 i-1]); %#ok<*SAGROW>
    ...
    vert_pat = [];
    % Create a vertical bar matrix
    base_vert_pat = [high_gs_value*ones(num_rows,vert_stripe_size), mid_gs_value*ones(num_rows,num_cols-vert_stripe_size)];
    % Vertically center the pattern at 47?
    base_vert_pat = circshift(base_vert_pat,[0 -2-vert_stripe_size/2]);
    for i = 1:num_cols
        vert_pat(:,:,i) = circshift(base_vert_pat,[0 i-1]); %#ok<*SAGROW>
    ...
    % Low contrast Bright bar
    Pats(:,:,:,4) = vert_pat;
    
    [~] = save_make_panelsV3_pattern(Pats,row_compression,gs_val,pattern_str,save_directory,count,testing_flag);

% Make a horzontal bar and vertical grat that moves up / down depending on
% the x displacement (Best so far)
%==========================================================================
    count = 998;
    pattern_str = '8px_vert_bar_4_px_horiz_windowed_up_down_grat_closed_loop_interspersal_pat';
    ...
    for x = 1:size(vert_pat,3)
        ...
        Pats(:,:,x,y) = (vert_pat(:,:,x,1)+horiz_pat(:,:,x,1));
        frame = Pats(:,:,x,y);
    ...
    % Overlay the two patterns
    y = 2;
    Pats(:,:,:,y) = repmat(high_frame,[1 1 size(Pats,3)]);
    for x = 1:size(vert_pat,3)
...
if 0
% Make a horzontal bar and vertical grat that moves up / down depending on
% the x displacement (this direction coupling causes more spinning than the
% other one ('up_down'))
%==========================================================================
    count = 700;
    pattern_str = '8px_vert_bar_4_px_horiz_windowed_down_up_grat_closed_loop_interspersal_pat';
...
% Make a horzontal bar and vertical bar (does not seem to work well)
%==========================================================================
    count = 701;
...
% Make a horzontal bar and vertical gratings (works okay...)
%==========================================================================
    count = 702;
