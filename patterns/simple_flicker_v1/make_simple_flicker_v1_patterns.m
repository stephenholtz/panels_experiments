...
for side = 1
    ...
    for pattern_type = 1:5
        clear pat
        switch pattern_type
            case 1
                stim_str = 'mid_to_on_off_flicker';
                % ON->OFF Flicker, the dummy frame here has to be the same as the first frame
                pat(:,:,1,1) = mid_gs*ones(num_rows,num_arena_cols);
                for pol = 1:2
                    if pol == 1;
                        pat(:,:,pol+1,1) = max_gs*ones(num_rows,num_arena_cols);
                    elseif pol == 2;
                        pat(:,:,pol+1,1) = min_gs*ones(num_rows,num_arena_cols);
                    end
                end
            case 2
                stim_str = 'mid_to_off_on_flicker';
                % OFF->ON Flicker
                pat(:,:,1,1) = mid_gs*ones(num_rows,num_arena_cols);
                for pol = 1:2
                    if pol == 1;
            ...
            case 3
                stim_str = 'off_to_on_off_flicker';
                % ON->OFF Flicker, the dummy frame here has to be the same as the first frame
                pat(:,:,1,1) = min_gs*ones(num_rows,num_arena_cols);
                for pol = 1:2
                    if pol == 1;
                        pat(:,:,pol+1,1) = max_gs*ones(num_rows,num_arena_cols);
                    elseif pol == 2;
            ...
            case 4
                stim_str = 'on_to_off_on_flicker';
                % OFF->ON Flicker
                pat(:,:,1,1) = max_gs*ones(num_rows,num_arena_cols);
                for pol = 1:2
                    if pol == 1;
                        pat(:,:,pol+1,1) = min_gs*ones(num_rows,num_arena_cols);
                    elseif pol == 2;
